 ...
 * Time : 10:33
 */
public final class NewEUAComputer implements IComputer<JSCATS> {
    ...
    public NewEUAComputer(Machine machine, List<AbstractState> abstractStates) {
        this(machine, abstractStates, false);
    }

    public NewEUAComputer(Machine machine, List<AbstractState> abstractStates, boolean usePropagation) {
    ...
    public NewEUAComputer(Machine machine, List<AbstractState> abstractStates, IRelevancyChecker relevanceChecker) {
        this(machine, abstractStates, relevanceChecker, false);
    }

    public NewEUAComputer(Machine machine, List<AbstractState> abstractStates, IRelevancyChecker relevanceChecker, boolean usePropagation) {
        this.machine = machine;
        this.abstractStates = abstractStates;
    ...
    public JSCATS compute() {
        ...
        //Map<AbstractState, List<Event>> E_ = new HashMap<>();
        ...
        /*for (AbstractState q : abstractStates) {
            E_.put(q, new ArrayList<>());
            for (Event e : machine.getEvents()) {
                z3.reset();
                z3.addCode(ExpressionToSMTLib2Formatter.formatExpression(new And(
                        machine.getInvariant(),
                        (ABooleanExpression) machine.getInvariant().prime(true),
                        (ABooleanExpression) q.prime(),
                        e.getSubstitution().getPrd(machine)
                )));
                if (z3.checkSAT() == Status.SATISFIABLE) {
                    E_.get(q).add(e);
            ...
            for (AbstractState qPrime : sortedAbstractStates) {
                for (Event e : E.get(q)) {
                    ...
                    if (nc.getFirst()) {
                        ...
                        if (!E.get(qPrime).isEmpty() && !Q.contains(qPrime)) {
                            RQ.add(qPrime);
                        }
                    }
                    //}
            ...
            while (!RC.isEmpty()) {
                ...
                for (Event e : E.get(Alpha.get(c))) {
                    ...
                    if (cPrimeSat == Status.SATISFIABLE) {
                        ...
                        //AbstractState abstractState = AbstractStateComputer.computeAbstractState(booleanExpression, machine, getAbstractStates().stream().filter(q -> E_.get(q).contains(e)).collect(Collectors.toList()));
